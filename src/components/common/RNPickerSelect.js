...
import { defaultStyles } from './RnPickerSelectStyle';
import { FloatingEditText } from '../../../src/components/common';
...
export default class RNPickerSelect extends PureComponent {
    ...
    renderTextInputOrChildren() {
        const { children, style, textInputProps ,placeholder,value} = this.props;
        ...
        return (
            <View pointerEvents="box-only" style={containerStyle}>
                ...
                <FloatingEditText style={[
                        Platform.OS === 'ios' ? style.inputIOS : style.inputAndroid,
                        this.getPlaceholderStyle(),
                    ]}
                    label={!value || value==-1?"": placeholder.label}
                    value={this.state.selectedItem.label}
                    ref={this.setInputRef}
                    editable={false}
                    {...textInputProps} 
                    textColor={ value==-1?Colors.NormalGreyColor:Colors.Defaultblack}
                    />
                {this.renderIcon()}
            </View>
        );
